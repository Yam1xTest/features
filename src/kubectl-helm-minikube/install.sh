...
if [ ${KUBECTL_VERSION} != "none" ]; then
    ...
    curl -sSL -o /usr/local/bin/kubectl "https://mirror.yandex.ru/mirrors/dl.k8s.io/${KUBECTL_VERSION}/bin/linux/${architecture}/kubectl"
    chmod 0755 /usr/local/bin/kubectl
    if [ "$KUBECTL_SHA256" = "automatic" ]; then
        KUBECTL_SHA256="$(curl -sSL "https://mirror.yandex.ru/mirrors/dl.k8s.io/${KUBECTL_VERSION}/bin/linux/${architecture}/kubectl.sha256")"
    fi
